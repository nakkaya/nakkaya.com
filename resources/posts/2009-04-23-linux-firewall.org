...
Same idea but only reject/allow on eth0, (used on the beagleboard
reject all on eth0 but do not mess with usb0). Accept all traffic
by default, allow incoming traffic on eth0 that is a part of a
connection we already allowed and allow incoming ssh, drop the rest.
