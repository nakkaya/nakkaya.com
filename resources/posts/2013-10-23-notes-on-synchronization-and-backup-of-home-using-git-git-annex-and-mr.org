...
#+BEGIN_SRC conf :tangle ~/annex/.mrconfig-annex
  ...
  [/home/nakkaya/annex/encryption-keys-git]
  checkout = git clone 'ssh://git@bitbucket.org/nakkaya/encryption-keys-git.git' 'encryption-keys-git'
             cd encryption-keys-git/
             enableCloud     
  skip = lazy
